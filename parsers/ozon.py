...
TAG_RE = re.compile(r"<[^>]+>")
TITLE_RE = re.compile(r"<[^>]*(?:title|name|tsbody|tile-hover-target)[^>]*>(.*?)</[^>]+>", re.IGNORECASE | re.DOTALL)
UNAVAILABLE_MARKERS = (
    "out of stock",
    "sold out",
    "\u043d\u0435\u0442 \u0432 \u043d\u0430\u043b\u0438\u0447\u0438\u0438",
...
def _looks_like_card(tag) -> bool:
    ...
    return any(marker in haystack for marker in ("tile-root", "product-card", "search-result", "widget-search-result"))


def _looks_like_title(tag) -> bool:
    haystack = _attr_haystack(tag)
    return any(marker in haystack for marker in ("title", "name", "tsbody", "tile-hover-target"))
