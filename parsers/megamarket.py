...
SEARCH_URL = "https://megamarket.ru/catalog/?q=rtx%205070%20ti"
BASE_URL = "https://megamarket.ru"
SOURCE = "Megamarket"
PRICE_RE = re.compile(r"(\d[\d\s\u00a0]{2,11})\s*(?:\u20bd|\u0440\u0443\u0431|RUB)", re.IGNORECASE)
CARD_RE = re.compile(r"<(?P<tag>[a-z0-9-]+)[^>]*(?:catalog-item|product-card|item-block|goods-item)[^>]*>.*?</(?P=tag)>", re.IGNORECASE | re.DOTALL)
HREF_RE = re.compile(r"href=[\"']([^\"']+)[\"']", re.IGNORECASE)
TAG_RE = re.compile(r"<[^>]+>")
TITLE_RE = re.compile(r"<[^>]*(?:title|name|item-title|goods-title)[^>]*>(.*?)</[^>]+>", re.IGNORECASE | re.DOTALL)
UNAVAILABLE_MARKERS = (
    "out of stock",
    "sold out",
    "\u043d\u0435\u0442 \u0432 \u043d\u0430\u043b\u0438\u0447\u0438\u0438",
...
def _looks_like_card(tag) -> bool:
    ...
    return any(marker in haystack for marker in ("catalog-item", "product-card", "item-block", "goods-item"))


def _looks_like_title(tag) -> bool:
    haystack = _attr_haystack(tag)
    return any(marker in haystack for marker in ("title", "name", "item-title", "goods-title"))
...
def parse_offers_browser() -> list[ProductOffer]:
    ...
    html = fetch_html_safe(
        SEARCH_URL,
        save_to="debug_html/megamarket.html",
        wait_selectors=["[class*=\"catalog-item\"]", "[class*=\"product-card\"]", "[class*=\"item-block\"]"],
        extra_delay_ms=2500,
    )
